#!/bin/bash
#######################################################################################################
### File Name : 3-2-0.render_eksctl_nodegroup_config.sh
...
if [ -z "$PROJECT_NAME" -o -z "$ENVIRONMENT" -o -z "$TEMPLATE_FILE" -o -z "$OUTPUT_FILE" ]; then
    echo "Usage: ./3-2-0.render_eksctl_nodegroup_config.sh <project_name> <environment> <template_filename> <output_filename>"
    echo "Example: ./3-2-0.render_eksctl_nodegroup_config.sh hellow dev eksctl_nodegroup_conf.yaml hellow-dev-eksctl_nodegroup_conf.yaml "
    exit 1
fi
...
# =========<<<< Main Logic Coding Area Marking Comment (end) >>>>======================================                                                                                                363,1         81%
