...
CLUSTER_NAME="eks-cluster-${PROJECT_NAME}-${ENVIRONMENT}"  # Cluster Name
getAccountID                                # Account ID 정보 - 필수 항목
echo -e "\n-------------------------\n"
echo "# 1. aws-loadbalancer-controller 도구 설치 및 환경설정"
echo "4-1.helm-install-aws-loadbalancer.sh $PROJECT_NAME $REGION_CODE $AWS_ACCOUNT_ID"
./4-1.helm-install-aws-loadbalancer.sh $PROJECT_NAME $REGION_CODE $AWS_ACCOUNT_ID
