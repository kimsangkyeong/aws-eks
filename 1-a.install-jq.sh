#!/bin/bash
#######################################################################################################
### File Name : 1-a.install-jq.sh
### Description : Install jq on the Bastion Server
### Information : * jq reference site
...
## Description : jq 제공하는site의 정보를 읽어서 설치 가능 버전 목록 전체 조회
## Information :
#############################################################################
getList()
{
    ...
    echo -e "------- \t-----------\t\t--------------------------------------------------" && \
    curl -sL https://jqlang.org/download/ | \
    grep -oE 'https://github.com/jqlang/jq/releases/download/jq-[0-9.]+/jq-[^" ]+' | \
    sort -u | \
    sed -E 's#.*/jq-([0-9.]+)/jq-(.*)#\1\t\2\t\0#' | \
    column -t -s $'\t'
}
...
getParamList()
{
    ...
    DOWNLOAD_URL=$(curl -sL https://jqlang.org/download/ | \
                   grep -oE 'https://github.com/jqlang/jq/releases/download/jq-[0-9.]+/jq-[^" ]+' | \
                   grep "/jq-$VER/" | \
                   grep "$OS_ARCH$" | head -n 1)
...
install-jq()
{
    ...
    curl -Lo jq $(getParamList $1 $2 $3 false) # 다운로드
    chmod +x ./jq  # 실행 권한 부여
    mkdir -p $HOME/bin && mv ./jq $HOME/bin/jq  #실행파일 디렉토리로이동
...
usage() {
    ...
    echo "Example: $0 linux 1.8.1 amd64"
    echo "  - os: linux, macos, windows"
    echo "  - version: e.g., 1.8.1, 1.8.0, 1.7 (jq supported versions)"
    echo "  - arch: amd64, arm64, i386"
    exit 1
}
...
if [[ ! "$VER" =~ ^[0-9]+\.[0-9]+(\.[0-9]+)?$ ]]; then
    echo "Error: 버전 형식이 올바르지 않습니다 ($VER). 예: 1.8.1"
    usage
fi
